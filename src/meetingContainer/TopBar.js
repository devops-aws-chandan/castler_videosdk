  ...
  return (
    <OutlineIconButton
      ...
      onClick={() => {
        ...
        setTimeout(() => {
          db.collection("meetingDetails").doc("Participant-" + mMeeting.meetingId).update({memberStatus: "Meeting End", meetingEndTime: Date().toLocaleString()});
        db.collection("meetingDetails").doc("Host-" + mMeeting.meetingId).update({memberStatus: "Meeting End", meetingEndTime: Date().toLocaleString()});
...
const TopBar = ({ topBarHeight }) => {
  ...
  return isTab || isMobile ? (
    ...
    >
          ...
          <>
            <img
              ...
              src={
                defaultBrandLogoUrl ||
                brandLogoURL ||
                `https://www.castler.com/wp-content/uploads/2021/08/cropped-favicon1.png`
              }
              onError={() => {
      ...
      <Box>
            <div style={{color:'#FBE7A1',fontFamily:'Century Gothic', fontSize:20}}> Know Your Product </div>
      </Box>
