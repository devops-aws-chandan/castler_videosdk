  ...
  const getParams = () => {
    ...
    if (paramKeys.brandingEnabled === "true") {
      ...
      if (!paramKeys.brandName || paramKeys.brandName.length === 0) {
        ...
        // );
        paramKeys.brandName = "Castler.com"
      }
    }
  ...
  return meetingIdValidation.isLoading ? (
    ...
    <MeetingAppProvider
      {...{
        ...
        memberName : name,
        memberType : paramKeys.askJoin === "true" ? "Participant" : "Host",
        memberStatus : "Waiting",
        meetingId : paramKeys.meetingId,
        meetingURL : paramKeys.joinScreenMeetingUrl,
        meetingStartTime : Date().toLocaleString(),
        meetingEndTime : "",
        participantCanLeave: paramKeys.askJoin === "true" ? false : true
      }}
    >
      <MeetingProvider
        config={{
